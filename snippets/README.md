...
if [[ ! -f "$_script" ]]; then
  gh api repos/eficode/envoke/contents/snippets/resolve-env-refs.sh?ref=<SHA> \
...
```bash
# .env
source <(gh api repos/eficode/envoke/contents/snippets/resolve-env-refs.sh?ref=<SHA> \
